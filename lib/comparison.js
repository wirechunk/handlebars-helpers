 ...
 * the [arithmetic operator][operators] to use. You may also
 ...
 * {{/contains}}
 * ```
 * @param {Array|String} `collection` The collection to iterate over.
...
helpers.contains = function(collection, value, startIndex, options) {
  ...
  let result;
  if (collection == null || value == null || !collection.indexOf) {
    result = false;
  } else {
    result = collection.indexOf(value, startIndex) !== -1;
  }
  return util.value(result, this, options);
};
...
/**
 * Returns true if the given `value` is truthy, the opposite of `falsey`.
 *
 * @param {any} `val`
...
helpers.ifEven = function(num, options) {
  return util.value(isEven(num), this, options);
};
...
helpers.ifNth = function(a, b, options) {
  const isNth = isNumber(a) && isNumber(b) && b % a === 0;
...
helpers.ifOdd = function(val, options) {
  return util.value(!isEven(val), this, options);
};
...
/**
 * Returns true if `val` is falsey (not truthy): that is, if the value
 * is false, null, undefined, NaN, 0, 0n (BigInt), or an empty string.
 * Works as a block or inline helper.
 *
 * ```handlebars
 * {{#not a}}
 *   If `a` is not truthy then this will be rendered.
 * {{/not}}
 * ```
 *
 * ```handlebars
 * <!-- given that "x" is null -->
 * {{not x}}
 * <!-- results in:  'true' -->
 * ```
 *
